...
To enable a feature, change the value to true:
  $cf['KU_INSTANTREDIRECT'] = true;
To disable a feature, change the value to false:
  $cf['KU_INSTANTREDIRECT'] = false;
...
if (!$cache_loaded) {
  // Database
    $cf['KU_DBTYPE']          = 'mysql';  // Database type. Valid values are mysql. 
              // PostgreSQL is also supported. Supported values are pgsql.
              // SQLite is also supported. Supported values are sqlite and sqlite2. SQLite will not use any database software, only a single file.
...
// SQL  database
if (!isset($kx_db) && !isset($preconfig_db_unnecessary)) {
  try {
    if (in_array(KU_DBTYPE, array('mysql', 'pgsql'))) {
      $kx_db = new PDO(KU_DBTYPE . ':host=' . KU_DBHOST . ';dbname=' . KU_DBDATABASE, KU_DBUSERNAME, KU_DBPASSWORD, array(
        PDO::ATTR_PERSISTENT => KU_DBUSEPERSISTENT,
        PDO::ATTR_ERRMODE => KU_DEBUG ? PDO::ERRMODE_EXCEPTION : PDO::ERRMODE_SILENT
      ));
    } else if (in_array(KU_DBTYPE, array('sqlite', 'sqlite2'))) {
      $kx_db = new PDO(KU_DBTYPE . ':' . KU_DBDATABASE, array(
        PDO::ATTR_PERSISTENT => KU_DBUSEPERSISTENT,
        PDO::ATTR_ERRMODE => KU_DEBUG ? PDO::ERRMODE_EXCEPTION : PDO::ERRMODE_SILENT
      ));
    } else {
      die(/* DB error goes here */);
    }
  } catch (PDOException $e) {
    $kx_db->rollBack();

    die('SQL database connection error: ' . $e->getMessage());
  }

  try {
    $stmt = $kx_db->prepare(sprintf('SELECT * FROM %sevents WHERE at <= ?', KU_DBPREFIX));
    $stmt->execute(array(time()));
    $results_events = $kx_db->fetchAll();
    unset($stmt);

    if (count($results_events) > 0) {
      foreach($results_events as $line_events) {
        if ($line_events['name'] == 'sitemap') {
          $stmt = $kx_db->prepare(sprintf('UPDATE %sevents SET at = ? WHERE name = "sitemap"', KU_DBPREFIX));
          $stmt->execute(array(time() + 21600));
          unset($stmt);
          ...
          if (KU_SITEMAP) {
            ...
            $stmt = $kx_db->query(sprintf('SELECT name, id FROM %sboards ORDER BY name ASC', KU_DBPREFIX));
            ...
            if (count($results) > 0) {
              foreach($results AS $line) {
                ...
                $stmt = $kx_db->prepare(sprintf('SELECT id, bumped FROM %sposts WHERE boardid = ? AND parentid = 0 AND IS_DELETED = 0 ORDER BY bumped DESC', KU_DBPREFIX));
    ...
    // Do nothing because if it fails I don't care (And that the original code didn't either)
  }
}
